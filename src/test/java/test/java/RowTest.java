...
public class RowTest {
    ...
    void cannotDrawAdjacentInterval() {
        // given
        LadderCreater ladderCreater = new LadderCreater(GreaterThanOne.from(2), GreaterThanOne.from(5));
        ladderCreater.drawLine(Position.from(0), Position.from(1)); // (1–2)

        // when & then
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class,
                () -> ladderCreater.drawLine(Position.from(0), Position.from(2))); // (2–3)
    ...
    void canDrawNonAdjacentIntervals() {
        ...
        // when & then
        assertDoesNotThrow(() -> ladderCreater.drawLine(Position.from(0), Position.from(2))); // (2–3)
    ...
    void drawLine_outOfRange_throwsInvalidPosition() {
        // given
        LadderCreater ladderCreater = new LadderCreater(GreaterThanOne.from(3), GreaterThanOne.from(4));
